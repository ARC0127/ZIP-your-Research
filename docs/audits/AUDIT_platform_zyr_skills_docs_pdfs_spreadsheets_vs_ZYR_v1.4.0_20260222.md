# Audit: `zyr_runtime_skills` (DOCX • PDF • Spreadsheet) → Reusable Templates → Alignment vs ZIP-your-Research (v1.4.0)

**Audit date:** 2026-02-22 (America/Los_Angeles)  
**Scope:** platform runtime guidance + scripts under `zyr_runtime_skills/**` (local filesystem audit).  
**Goal:** 查漏补缺：把 `zyr_runtime_skills` 全部文件做成 **可移植模板** + **与 ZYR 协议的对齐差异**，并落到 repo 内文档，避免“只存在于运行环境”的知识丢失。  
...
- `zyr_runtime_skills` 是**运行环境注入的工作流规范与示例**，不属于 ZYR release zip。若不沉淀为 repo 文档，会导致：
...
```text
# sha256sum (relative to zyr_runtime_skills)
...
- 我们完成了 `zyr_runtime_skills` 的全量清单化审计，并把“可移植模板 + 对齐差异”落到 repo 文档。
