---
name: zip-your-research
description: Route ZIP-your-Research (ZYR) tasks, start an explicitly requested ZIP workflow, or maintain the ZYR suite. Named Sxxx tasks use their exact skill.
version: 1.6.6
---

# ZIP Your Research

Suite v1.6.6 with resource-proportional execution profile v1.
The local suite root is `references/upstream/ZIP-your-Research-main/`.

Read `references/resource_policy.md` unless the same resource profile v1 is
already loaded through another ZYR entry. Use one primary skill
and read only its applicable protocol. Search `skills_manifest.yaml` at the
suite root for an exact Sxxx path; do not preload README, INDEX, the complete
manifest, every boot file, or multiple engines.

- Use `router/route_v1_8.py` at the suite root for active routing.
- Local prose edits use the relevant writing skill; mathematical validity uses
  proof verification; integrated manuscript work retains its global checks.
- Ordinary authoritative retrieval uses the current agent. S660 is for an
  explicitly requested multi-agent research or independent review workflow.
- For suite maintenance, use `zyr-research-evolution`; do not invoke S660 merely
  because the task says "optimize" or "improve".
- For ZIP-only startup or an explicitly requested strict boot, read the active
  banner, guardrails, and compatibility bootstrap under `boot/`, then retain
  the existing CONFIRM requirement. Atomic skill use does not restart intake.
- S661 concerns generated procedural memory. Preserve its exact consent,
  host-attestation, and evaluation requirements when that workflow is selected.

Preserve source protocols and user-frozen facts. Mark unsupported claims UNKNOWN.
Do not claim unread sources or unexecuted checks. Installed source maintenance
requires the user's scope authorization, a baseline backup, and validation.
