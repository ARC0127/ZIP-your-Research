...
> **Component lineage:** the preserved writing modules originate from the v1.3.2-compatible prompt line; the active suite and this entrypoint are v1.6.6.

> **v1.6.6 binding addendum:** Any visible writing task in ZYR must call `writing_engine`. In the v1.6.6 stack, `writing_engine` is backed by `ext/src/rpws/` (Research-Paper-Writing-Skills) and must use the integrated wrappers `S601`/`S602`/`S603`/`S604` together with `S640` as the global writing and logic gate.
