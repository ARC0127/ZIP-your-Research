...
> **Active applicability (resource profile v1):** Follow `boot/14_RESOURCE_PROPORTIONAL_EXECUTION_v1.md`. It scopes the blanket "any task" and mandatory companion language retained below. Select the protocol needed for the requested result; companion lists are navigation until a distinct obligation requires them. Local wording does not require a proof audit; mathematical validity does. Integrated manuscript work retains the relevant global checks. Read only the applicable writing_engine modules, and preserve their scientific and output contracts.

> This file is generated by `python tools/zyr.py build`.

> **Component lineage:** the preserved writing modules originate from the v1.3.2-compatible prompt line; the active suite and this entrypoint are v1.7.0.

> **v1.7.0 binding addendum:** Any visible writing task in ZYR must call `writing_engine`. In the v1.7.0 stack, `writing_engine` is backed by `ext/src/rpws/` (Research-Paper-Writing-Skills) and must use the integrated wrappers `S601`/`S602`/`S603`/`S604` together with `S640` as the global writing and logic gate.
