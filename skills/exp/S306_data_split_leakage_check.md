...
outputs_required:
- Split policy (unit of split, time, identity)
- Leakage checklist (data, labels, metadata)
- Automatable tests (pseudo-code allowed)
- Remediation plan if leakage found
quality_gates:
- no fabrication
- mark UNKNOWN
- decision-oriented
...
1) Split policy (unit of split, time, identity)
2) Leakage checklist (data, labels, metadata)
3) Automatable tests (pseudo-code allowed)
4) Remediation plan if leakage found
