...
> **v1.6.5 binding addendum:** Any research-idea construction, method design, theoretical framing, contribution definition, paper-storyline construction, theorem/proof sketch, derivation, or claim-evidence task must call `proof_engine` before prose polishing. In the v1.6.5 stack, `proof_engine` is the reasoning gate that stabilizes the idea-method-evidence chain before `writing_engine` turns it into polished text.


> This file is generated by `tools/build_proof_engine_v1_5.py`.  
